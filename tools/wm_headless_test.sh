...
#   -> DOWN DOWN ENTER (select Graphic) -> expect "[WM] count=" in serial, no EXCEPTION.
...
if ! grep -q "\[WM\] count" "$constSER"; then
  echo "RESULT: FAIL (did not reach window system / no [WM] count)"
  exit 1
fi
echo "RESULT: OK"